...
1. ~~**Azul primário duplicado.**~~ **Resolvido.** O sidebar/logo usava `--fp-accent` (`#4361EE`), mas a maioria dos botões inline usava `indigo-600` do Tailwind (`#4F46E5`) — duas cores de marca "azul primário" diferentes. Corrigido sobrescrevendo a escala `indigo` inteira no `tailwind.config.js` para resolver aos valores de `blue` (token `action.primary.*`), sem precisar tocar em cada view.
2. ~~**Verde de sucesso duplicado.**~~ **Resolvido.** `--fp-success` (`#2EC4B6`, teal) não era usado em nenhuma view; as views usavam `emerald-*`. `--fp-success` agora aponta para `emerald-600` (`#059669`) e `.fp-alert-success` usa os mesmos tons emerald das views (token `feedback.success.*`).
3. ~~**Vermelho de erro duplicado.**~~ **Resolvido.** Mesma situação do azul: `--fp-danger` (`#E63946`) nos componentes `.fp-*`, mas `red-*` cru do Tailwind nas views inline. Corrigido sobrescrevendo a escala `red` no `tailwind.config.js` para resolver a partir de `#E63946` (token `feedback.danger.*`).
4. **Dois sistemas de estilo coexistindo.** Um baseado em classes semânticas `.fp-*` + CSS vars (layout, auth, sidebar) e outro em utilitárias Tailwind cruas (`slate`/`indigo`/`emerald`/`red`) usadas diretamente nas telas de conteúdo (transações, categorias, recorrentes). As cores das duas camadas já apontam para os mesmos tokens agora (achados 1-3); resta unificar a *arquitetura* (Blade components reutilizáveis) — ver Fase 6 do `FIGMA_SYNC_ROADMAP.md`.